...
tuple타입을 받는 함수에 변수를 생성하지 않고 전달하기 위해 사용합니다.
...
**forward_as_tuple로 인자를 넘겨준다면 rvalue references(우측값 참조)로 넘겨준다는** 특징이 있습니다.
...
"hello"처럼 단순 char배열을 넘겨줄 경우, rvalue참조 string타입으로 자동변환이 안되기 때문에 정상적인 작동을 하지 않습니다.
...
그 값이 rvalue string이기때문에 정상적으로 작동합니다.
...
알고리즘 문제를 풀 때, sort함수와 같이 사용하면 유용합니다.
...
를 하면 tuple변수에 담긴 순서를 기준으로 정렬을 해줍니다. 
...
**두 번째**에 **가입번수**,알고리즘 문제를 풀 때, sort함수와 같이 사용하면 유용합니다.
